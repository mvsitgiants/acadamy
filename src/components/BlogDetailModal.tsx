...
export const BlogDetailModal: React.FC<BlogDetailModalProps> = ({ blog, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white rounded-t-3xl sm:rounded-3xl max-w-2xl w-full overflow-hidden shadow-2xl border border-slate-200 flex flex-col max-h-[92dvh] sm:max-h-[90vh]">
        ...
        <div className="p-5 sm:p-6 overflow-y-auto overscroll-contain space-y-4">
          <div className="h-44 sm:h-56 rounded-2xl overflow-hidden bg-slate-100">
            <img 
              src={blog.image} 
              alt={blog.title} 
